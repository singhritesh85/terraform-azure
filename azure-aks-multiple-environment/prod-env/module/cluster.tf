...
resource "azurerm_log_analytics_workspace" "insights" {
  name                = "${var.prefix}-log-analytics-workspace-${var.env}"
...
resource "azurerm_monitor_workspace" "azure_monitor_workspace" {
  name                = "${var.prefix}-monitor-workspace-${var.env}"
  resource_group_name = azurerm_resource_group.aks_rg.name
  location            = azurerm_resource_group.aks_rg.location
}
...
resource "azurerm_monitor_data_collection_endpoint" "azure_monitor_datacollection_endpoint" {
  name                = "${var.prefix}-monitor-datacollection-endpoint-${var.env}"
  resource_group_name = azurerm_resource_group.aks_rg.name
  location            = azurerm_resource_group.aks_rg.location
  kind                = "Linux" 
}
...
resource "azurerm_monitor_data_collection_rule" "azure_monitor_datacollection_rule" {
  name                        = "${var.prefix}-monitor_datacollection-rule-${var.env}"
  resource_group_name         = azurerm_resource_group.aks_rg.name
  location                    = azurerm_resource_group.aks_rg.location
...
resource "azurerm_virtual_network" "aks_vnet" {
  ...
  resource_group_name = azurerm_resource_group.aks_rg.name
  location            = azurerm_resource_group.aks_rg.location
  address_space       = ["10.96.0.0/12"]
}

# Create Subnet for VNet of AKS
resource "azurerm_subnet" "aks_subnet" {
  name                 = "default"                   ###"${var.prefix}-subnet-${var.env}"
  resource_group_name  = azurerm_resource_group.aks_rg.name
  service_endpoints    = ["Microsoft.ContainerRegistry"]
  virtual_network_name = azurerm_virtual_network.aks_vnet.name
  address_prefixes     = ["10.96.0.0/16"]
...
resource "azurerm_kubernetes_cluster" "aks_cluster" {
  ...
  location            = azurerm_resource_group.aks_rg.location
  resource_group_name = azurerm_resource_group.aks_rg.name
  dns_prefix          = "${var.prefix}-${var.env}-cluster-dns"
  kubernetes_version  = var.kubernetes_version
  node_resource_group = "${var.prefix}-noderg-${var.env}"
  sku_tier            = "Standard"
...
# Network Profile
  network_profile {
    ...
    service_cidr        = "10.65.0.0/16"  ### Kubernetes service address range
    dns_service_ip      = "10.65.0.10"    ### Kubernetes DNS service IP address
  }
...
resource "azurerm_monitor_data_collection_rule_association" "azure_monitor_dcr_to_aks" {
  name                    = "${var.prefix}-dcr-to-aks-${var.env}"
...
resource "azurerm_dashboard_grafana" "azure_grafana" {
  name                = "${var.prefix}-grafana-${var.env}-${random_id.id.hex}"
  resource_group_name = azurerm_resource_group.aks_rg.name
  location            = azurerm_resource_group.aks_rg.location
...
resource "azurerm_monitor_action_group" "action_group" {
  name                = "${var.prefix}-action-group-${var.env}"
  resource_group_name = azurerm_resource_group.aks_rg.name
  location            = "global"
...
resource "azurerm_monitor_metric_alert" "alert_rule1" {
  name                = "${var.prefix}-alert-rule1-${var.env}"
  resource_group_name = azurerm_resource_group.aks_rg.name
...
resource "azurerm_monitor_metric_alert" "alert_rule2" {
  name                = "${var.prefix}-alert-rule2-${var.env}"
  resource_group_name = azurerm_resource_group.aks_rg.name
  scopes              = [azurerm_kubernetes_cluster.aks_cluster.id]
  auto_mitigate       = true    ### Metric Alert to be auto resolved
  frequency           = "PT5M"
